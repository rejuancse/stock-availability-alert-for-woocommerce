    ...
    public function add_notify_me_button() {
        ...
        if (!$product->is_in_stock()) {
            echo '<div class="notify-me-button-wrap">';
                echo '<button class="button" id="notify-me-button">' . esc_html__('Notify Me When Available', 'stock-availability-alert') . '</button>';
                echo '<div id="notify-me-form" style="display: none;">
                        <div class="form-fields">
                            <input type="email" id="notify-email" placeholder="' . esc_attr__( 'Enter your email', 'stock-availability-alert' ) . '" required>
                            <input type="hidden" id="notify-product-id" value="' . esc_attr( $product->get_id() ) . '">
                            <button id="submit-notify">' . esc_html__( 'Notify Me', 'stock-availability-alert' ) . '</button>
                        </div>
                    </div>';
            echo '</div>';
        }
